function calculate() {
  ...
  let result =
    userAge < 18
      ? " too young"
      : userAge <= 27
      ? " right for military service"
      : userAge <= 41
      ? " reserve for military"
      : userAge <= 55
      ? " in backup for military"
...
function calculate() {
  let hSalary = Number(document.querySelector("#salary").value);
  let hours = Number(document.querySelector("#hour").value);
  let solution = document.querySelector("#salaryCalculate");
